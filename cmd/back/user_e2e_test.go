...
func TestE2E(t *testing.T) {
	...
	assert, grpcClient, cfg := initService(t, ctx)
	...
	ctx = authCtx(ctx, strings.Join(md.Get("authorization"), ""))
	...
	addr := fmt.Sprintf("http://%s:%d/user/api/v1/file", cfg.Server.Host, cfg.Server.Port.Files)
	...
	_, err = grpcClient.AddAvatar(ctx, &user_pb.AddAvatarRequest{FileId: getFile.FileID.String()})
	assert.NoError(err)
